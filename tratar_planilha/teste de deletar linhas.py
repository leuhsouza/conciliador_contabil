...
def process_excel(file_path, output_path):
    ...
    while current_index < len(df):
        ...
        index_to_delete = df.iloc[current_index:, 3].first_valid_index()
        #index_int = int(index_to_delete) +1 if index_to_delete is not None else +1
        #print(index_int)
        if index_to_delete is None:
            break

        #Calcular o intervalo de linhas a serem deletadas
        rows_to_delete = list(range(index_to_delete, index_to_delete + 7))
        print(f"Índices a serem deletados: {rows_to_delete}")
