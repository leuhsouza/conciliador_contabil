...
def process_excel(file_path, db_path, output_path=None, enviar_bd=False):
    ...
    # Processamento dos dados (mesma lógica existente)
    current_index = 0
    ...
    linhas_importadas = 0  # Variável para contar as linhas importadas
    ...
    if enviar_bd:
        ...
        for row in df.itertuples(index=False, name=None):
            print(f"Tentando inserir linha: {row}")  # Adicionado para depuração
            cursor.execute('''
                INSERT OR IGNORE INTO dados (id, coluna1, coluna2, coluna3, coluna4, coluna5, coluna6, coluna7, coluna8, coluna9)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (next_id, *row))
            print(f"Rowcount após inserção: {cursor.rowcount}")  # Adicionado para depuração
            if cursor.rowcount > 0:  # Se a linha foi inserida com sucesso
                next_id += 1
                linhas_importadas += 1  # Incrementa a contagem de linhas importadas
    ...
    # Retorna o número de linhas importadas para feedback
